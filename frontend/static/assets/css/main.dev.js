...
(function ($) {
  ...
  function mainMenu() {
    ...
    var var_window = $(window),
        ...
        closeIcon = $('.navbar-close'); // navbar toggler
  ...
  if ($(".swiper-certi").length) {
    $(".swiper-certi").each(function () {
      var $ttCertificate = $(this); // Init Swiper
      // =============
  ...
  $('select').niceSelect(); // Scrolling button
  // =================
  ...
  $(window).scroll(function () {
    ...
    var $sub = $(".section-color"),
        $window = $(window),
        $main = $("body");
    var $scroll = $window.scrollTop() + $window.height() / 3; //for each element with class ".sub"

    $sub.each(function () {
      var $this = $(this); //check if it is being viewed

      if ($this.position().top <= $scroll && $this.position().top + $this.height() > $scroll) {
        //if is viewed: get its id
        //store it in className var
        var className = $this.attr("id"); //remove color classes from the 'main' div
